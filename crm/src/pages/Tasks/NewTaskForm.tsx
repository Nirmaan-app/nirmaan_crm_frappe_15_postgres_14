...
export const NewTaskForm = ({ onSuccess }: NewTaskFormProps) => {
  ...
  const { data: allCompanies, isLoading: companiesLoading } = useFrappeGetDocList<CRMCompany>("CRM Company", { fields: ["name", "company_name"],limit:0, enabled: !companyIdFromContext });
  const { data: contactsList, isLoading: contactsLoading } = useFrappeGetDocList<CRMContacts>("CRM Contacts", { filters: { company: selectedCompanyByUser || companyId }, fields: ["name", "first_name", "last_name"],limit:0, enabled: !!(selectedCompanyByUser || companyId) && !contactIdFromContext });
  const { data: boqsList, isLoading: boqsLoading } = useFrappeGetDocList<CRMBOQ>("CRM BOQ", { filters: { contact: selectedContactByUser || contactIdFromContext }, fields: ["name", "boq_name"], limit: 0, enabled: !!(selectedContactByUser || contactIdFromContext) });
