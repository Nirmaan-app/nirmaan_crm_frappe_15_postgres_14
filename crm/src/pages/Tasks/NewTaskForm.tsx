...
import { salesTaskTypeOptions } from "@/constants/dropdownData"; // --- CHANGE 1: Import sales-specific options ---
...
export const NewTaskForm = ({ onSuccess }: NewTaskFormProps) => {
  ...
  // --- CHANGE 3: The `task_profile` is now part of the context ---
  const { companyId: companyIdFromContext, contactId: contactIdFromContext, boqId: boqIdFromContext, taskId: taskIdFromContext, task_profile } = newTask.context;
  // console.log("newTask.context",newTask.context)
  const form = useForm<TaskFormValues>({
    resolver: zodResolver(taskFormSchema),
    defaultValues: {
      task_profile: task_profile || "Sales" // Pre-fill from context or default to Sales
    },
  });
  ...
  useEffect(() => {
    form.reset({
      ...
      task_profile: task_profile || "Sales", // Ensure profile is set on reset
      type: "", start_date: "",
      //  time: "", 
  ...
  const onSubmit = async (values: TaskFormValues) => {
    try {
      const payload = {
        ...values,
        status: "Scheduled" // Always set status on creation
      };
