...
// // NOTE: We have removed the imports for Badge and Select components.
...
//     "company.company_name": string;
//     salesperson?: string;
//     modified: string; // Ensure 'modified' is in the interface
// }
...
//         <div className="bg-background p-4 rounded-xl border">
//             <h2 className="font-semibold text-lg mb-4">Pending BOQs - {isLoading ? '...' : data?.length ?? 0}</h2>
//             <div className="hidden md:grid md:grid-cols-[2fr,1fr,1fr,1fr,1fr] gap-4 font-medium text-sm text-muted-foreground px-2 mb-2">
//                 <span>Project Name</span>
//                 <span>Current Status</span>
//                 <span>Sub-Status</span>
//                 <span>Submission Date</span>
...
//                 {isLoading && Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-20 w-full" />)}
//                 {!isLoading && data?.map(boq => (
//                     <div key={boq.name} className="flex justify-between items-start md:grid md:grid-cols-[2fr,1fr,1fr,1fr,1fr] md:items-center p-3 border rounded-lg md:border-none md:p-2">

//                         {/* --- LEFT SIDE (MOBILE) / FIRST COLUMN (DESKTOP) --- */}
//                         <div className="flex flex-col">
//                             <button onClick={() => navigate(`/boqs/boq?id=${boq.name}`)} className="text-primary font-semibold hover:underline text-left">{boq.boq_name}</button>
//                             <p className="text-xs text-muted-foreground">Created By: {boq.owner}</p>
...
//                         <div className="hidden md:block">
//                            {boq.boq_sub_status ? (
//                                <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${getBoqStatusClass(boq.boq_sub_status)}`}>
//                                    {boq.boq_sub_status}
//                                </span>
//                            ) : '--'}
//                         </div>
...
//                         {/* --- RIGHT SIDE (MOBILE) / LAST COLUMN (DESKTOP) --- */}
//                         <div className="flex flex-col items-end gap-2 md:text-center">
//                             <div className="flex flex-col items-end gap-2 md:hidden">
//                                 <span className={`text-xs font-semibold px-3 py-1 rounded-full ${getBoqStatusClass(boq.boq_status)}`}>
//                                     {boq.boq_status}
...
//     variant="destructive" // Handles the red color and hover effect
//     size="sm"             // Makes the button smaller
//     className="mt-2"      // Keeps only the necessary margin
// >
//     Update BOQ Status
// </Button>
//                         </div>
...
//         const companies = new Map<string, string>();
//         allBoqs.forEach(boq => {
//             if (boq.company && boq["company.company_name"]) {
//                 companies.set(boq.company, boq["company.company_name"]);
//             }
//         });
//         return Array.from(companies.entries()).map(([id, name]) => ({ id, name }));
//     }, [allBoqs]);
...
//             const searchMatch = searchTerm.toLowerCase() === '' ||
//                 boq.boq_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
//                 boq["company.company_name"]?.toLowerCase().includes(searchTerm.toLowerCase()) ||
//                 boq.owner.toLowerCase().includes(searchTerm.toLowerCase());
//             return companyMatch && searchMatch;
//         });
//     }, [allBoqs, searchTerm, companyFilter]);
...
//                     className="w-full md:w-[200px] border border-input bg-background h-10 rounded-md px-3 text-sm ring-offset-background focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2"
//                 >
//                     <option value="all">All Companies</option>
...
//              </div>
//             <div>
//                 <div className="hidden md:grid md:grid-cols-[2fr,1fr,1fr,1fr,1fr,1fr,auto] gap-4 font-medium text-sm text-muted-foreground px-2 mb-2">
//                     <span>Project Name</span>
//                     <span>Company Name</span>
//                     <span>Current Status</span>
//                     <span>Sub-Status</span>
...
//                     {isLoading && Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-20 w-full" />)}
//                     {!isLoading && filteredBoqs.map(boq => (
//                         <div key={boq.name} onClick={() => navigate(`/boqs/boq?id=${boq.name}`)} className="flex justify-between items-start md:grid md:grid-cols-[2fr,1fr,1fr,1fr,1fr,1fr,auto] md:items-center p-3 border rounded-lg cursor-pointer hover:bg-muted/50 transition-colors">
//                             {/* --- LEFT SIDE (MOBILE) / MULTIPLE COLUMNS (DESKTOP) --- */}
//                             <div className="flex flex-col text-left">
//                                 <p className="font-semibold text-primary">{boq.boq_name}</p>
//                                 <p className="text-sm text-muted-foreground md:hidden">{boq["company.company_name"] || boq.company || '--'}</p>
//                                 <p className="text-xs text-muted-foreground md:hidden">By: {boq.owner} </p>
                                 
//                             </div>

//                             {/* --- DESKTOP-ONLY CELLS --- */}
//                             <span className="hidden md:block text-left">{boq["company.company_name"] || boq.company || '--' }</span>
//                             <div className="hidden md:block text-left">
//                                 <span className={`text-xs font-semibold px-3 py-1 rounded-full ${getBoqStatusClass(boq.boq_status)}`}>{boq.boq_status}</span>
//                             </div>
//                             <div className="hidden md:block text-left">
