...
//       <Button
//         variant="outline"
//         className="absolute left-10 h-7 w-7 bg-transparent p-0 opacity-80 hover:opacity-100"
//         type="button"
...
import {
  ...
  useDayPicker,
  type DayPickerProps,
  DateRange // Import DateRange type
} from "react-day-picker"

// ... (Your CalendarProps and other parts of the file remain the same) ...
export type CalendarProps = DayPickerProps & {
  yearRange?: number
  showYearSwitcher?: boolean
  monthsClassName?: string
          ...
          return (
            <Button
              ...
              onClick={() => {
                ...
                // Instead of `new Date(selected)`, we now safely access `selected.from`.
                // If `selected` or `selected.from` doesn't exist, we fall back to 0 (January).
                const month = (selected as DateRange | undefined)?.from?.getMonth() ?? 0;
